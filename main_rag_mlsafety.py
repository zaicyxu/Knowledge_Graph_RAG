...
class Neo4jRAGSystem:
    ...
    def update_embeddings(self):
        ...
        with self.driver.session() as session:
            ...
            """
            target_labels = ["ML_Flow", "ML_Safety_Requirement", "Sensors", "System_Description",
    ...
    def analyze_query_intent_with_LLM(self, query_text):
        ...
        Minimal change: update the system prompt entities/relations to match the new dataset labels.
        """
        import json
        ...
        1. The main entity type(s) being queried (choose from: ML_Flow, ML_Safety_Requirement, Sensors, System_Description, System_Safety_Requirement, actuators, algorithms, functional, functionalility).
        2. The relationship types needed to answer the question (choose from: NEXT, Input, Output, Consist, Include, Serve, Collect_Data, Dataflow).
        3. Any specific filtering conditions (e.g., requiring a specific component name).

        Return a JSON response in the following format:
        {
            "entities": ["ML_Flow"],
            "relationships": ["NEXT"],
            "filters": {"name_contains": "sensor"}
        }
        """

        # keep the original model call pattern
        response = model.generate_content(system_prompt + "\nUser question: " + query_text)

        # try to parse JSON from the model output; if it fails, return safe empty lists
        try:
            return json.loads(response.text)
        except Exception:
            # fallback: return empty selections so downstream logic does not assume an obsolete default
            return {"entities": [], "relationships": [], "filters": {}}

    def retrieve_relevant_entities(self, query_text, top_k=5, similarity_threshold=0.35, depth=2):
        """
        Retrieve relevant entities from Neo4j and return them in the expected schema:
        [
          {
            "node1": {"id": <int>, "name": <str>, "type": <list_of_labels>},
            "relations": [<relation-types>],
            "connected_nodes": [{"id":<int>, "name":<str>, "type":<labels>}, ...]
          },
          ...
        ]
        """
        ...
        # --- Keyword-based Cypher (match labels, name, type) ---
        cypher_template = """
        MATCH p = (n)-[*1..{DEPTH}]-(m)
        ...
        LIMIT 200
        """
        cypher_query = cypher_template.replace("{DEPTH}", str(depth))
    ...
    def _to_prolog_atom(self, text, fallback=None):
        ...
        - Accepts None or non-string inputs.
        - If text is None, uses fallback if provided, otherwise returns 'unknown'.
        - Replaces spaces with underscores, lowercases, replaces invalid chars.
        - If final token cannot be a bare atom, returns a quoted atom with original text.
        """
        import re
    ...
    def generate_answer(self, user_question, relevant_entities):
        ...
        Robust to missing node names (uses id-based fallback) and skips malformed entities.
        """
        ...
        # --- Generate Prolog facts from retrieved entities (use safe predicate names) ---
        prolog_facts = []
        for entity in relevant_entities:
            try:
                # node1 may be missing keys; be defensive
                node1 = entity.get('node1', {}) if entity else {}
                node1_id = node1.get('id')
                # prefer name, otherwise fallback to id-based name
                ...
                raw_e1_label = None
                if node1.get('type'):
                    # type often stored as list of labels
                    t = node1.get('type')
                ...
                # connected nodes: be defensive about missing name/type
                ...
                for idx, node in enumerate(connected):
                    ...
                    # relation mapping: keep same index mapping if available
                ...
                # don't crash whole pipeline for one bad entity; log and continue
                print(f"[WARN] Skipping malformed entity while building Prolog facts: {e}")
                continue

        prolog_facts = sorted(set(prolog_facts))

        # --- Build LLM prompt (same structure, but few-shot updated for dataset) ---
        input_text = f"""You are an expert AI assistant specializing in knowledge graph analysis.
                    ...
                    1. The Prolog facts provided in the "Knowledge Context" section MUST be included exactly as given — do not modify, replace with variables, or reorder them.
                    2. After listing the Prolog facts, you may optionally add additional Prolog rules if they are strictly needed to answer the question.
                    3. Use lowercase predicate names and safe Prolog atoms for names (facts already follow this).
                    4. Immediately following the Prolog facts/rules, provide a detailed process reasoning section that outlines your step-by-step inference process.
                    5. If no direct matches exist, hypothesize potential relationships using analogical reasoning.

                    Extracted Keywords:
                    {', '.join(keywords)}

                    Knowledge Context (Prolog Facts - DO NOT MODIFY):
                    {chr(10).join(prolog_facts)}
                    """

        # keep the existing few-shot example (unchanged)
        input_text += """
                    ### Example:
                    User Question: Which sensors collect data for the ML flow 'anomaly_detection' and which algorithms process that flow?
                    Knowledge Context:
                    ml_flow('anomaly_detection').
                    sensors('vibration_sensor_1').
                    collect_data('vibration_sensor_1', 'anomaly_detection').
                    algorithms('random_forest_v1').
                    dataflow('random_forest_v1', 'anomaly_detection').

                    Expected Answer (Prolog facts, then reasoning):
                    collect_data('vibration_sensor_1', 'anomaly_detection').
                    algorithms('random_forest_v1').
                    dataflow('random_forest_v1', 'anomaly_detection').

                    Reasoning:
                    1) 'vibration_sensor_1' is listed as a Sensors node that collects data for ML flow 'anomaly_detection', therefore collect_data(...) holds.
                    2) 'random_forest_v1' is listed as an algorithms node connected by a Dataflow relation to 'anomaly_detection', so it processes that flow.
                    3) Combine these facts to conclude which sensors feed the flow and which algorithms process it.
                    """
